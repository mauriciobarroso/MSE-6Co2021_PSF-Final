...
header = { "pre": b"*header*", "id": 0, "N": 256, "fs": 10000, "cutFreq1": 0, "cutFreq2": 1, "M":10, "C": 12, "pos": b"end*"}
fig    = plt.figure ( 1 )
fig.suptitle('TP final PSF - Mauricio Barroso Benavides', fontsize=16)

#--------------------------ADC--------------------------
adcAxe = fig.add_subplot ( 2,1,1                  )
adcLn, = plt.plot        ( [],[],'g-o',linewidth=1, alpha = 0.3 ,label  = "adc")
adcAxe.grid              ( True                   )
...
fftAxe.grid     ( True   )
cutFreqPosZoneLn   = fftAxe.fill_between([0,0],100,-100,facecolor = "yellow",alpha = 0.2)
cutFreqNegZoneLn   = fftAxe.fill_between([0,0],100,-100,facecolor = "yellow",alpha = 0.2)
...
def findHeader(f,h):
    ...
    h["N" ]      = readInt4File(f)
    h["fs"]      = readInt4File(f)
    h["cutFreq1"] = readInt4File(f)
    h["cutFreq2"] = readInt4File(f)
    h["M"]       = readInt4File(f)
    h["C"]       = readInt4File(f)
    ...
    return h["id"],h["N"],h["fs"],h["cutFreq1"], h["cutFreq2"], h["M"]
...
def update(t):
    ...
    id, N, fs, cutFreq1, cutFreq2, M = findHeader(streamFile, header)
...
#    fftAxe.set_ylim ( 0,np.clip(np.max(ciaaFFT),0.01,100))
    ...
    cutFreqPosZoneLn = fftAxe.fill_between([cutFreq1, cutFreq2], 100, -100,facecolor="yellow",alpha=0.5)
    cutFreqNegZoneLn = fftAxe.fill_between([-cutFreq1, -cutFreq2], 100, -100,facecolor="yellow",alpha=0.5)

    return adcLn, ciaaFftLn, fftLn, cutFreqPosZoneLn, cutFreqNegZoneLn
